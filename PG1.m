...
% W_list is used in the actual MSE measurement 
...
data = generate_zipf(N_user,N_loc);
...
for i = 1:N_epsilon
    epsilon = myEpsilon(i);
    temp = W*epsilon;
    % here temp will containt 1 , 1.2, 2 times epsilon 
    ...
    [X, result_min(1,i)] = min_opt0(temp,fun0);     % what is the X here? result_min was used to generate the emperical reading on  graphs 
    Xmin0(:,i) = X;  
    a = X(1:N_lev);
    b = X((N_lev+1):end);
