...
x0 = [0.5*ones(N_loc,1); zeros(N_loc,1) ; ones(N_loc,1)];
...
for i = 1:N_loc
    for j = 1:N_loc
        ...
        q = 1 / (exp(W(i)) + d -1);
        c(row(i,j)) = (p)*b(i)*(1-a(j)) + q*a(i)*(1-b(j)) - q*exp(min(W(i),W(j)))*b(i)*(1-a(j)); % CORRECTED THE ERROR FOR q 
        %c(row(i,j)) = (p/q)*(d-1)*b(i)*(1-a(j)) + a(i)*(1-b(j)) - exp(min(W(i),W(j)))*b(i)*(1-a(j)); % NO CONSTRAINTS FROM Alpha or BETA
